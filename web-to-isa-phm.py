...
def create_isa_data(IsaPhmInfo: dict, output_path: str = None) -> Investigation:
    ...
    for study_index, study in enumerate(studies, start=1):
        ...
        for assay in study.get("assay_details", []):
            ...
            # raw_file_name may be empty (e.g. when the dataset contains only processed/derived
            # time-series CSV files). In that case we skip the Raw DataFile entirely so the
            # ISA graph reads:  Sample → [measurement] → ProcessedDataFile → [processing] → ProcessedDataFile
            # rather than creating an unnamed Raw DataFile node.
            runs = assay.get("runs", [])
            run_has_raw = []  # parallel list: True if this run has a raw file
            for run in runs:
            ...
            # Processed DataFiles are always created; track their index per run
            ...
            for run_idx, run in enumerate(runs):
                ...
                processed_data_file = DataFile(
                    filename=(run.get("processed_file_name") or "").strip(),
            ...
            for index, run in enumerate(runs):
                ...
                for protocol in study_obj.protocols:
                    ...
                    if (protocol.name == expected_measurement_name or 
                        ...
                        process.name = f"{sensor_alias}_run_{run_number}_measurement"
                        process.inputs.append(study_obj.samples[index])
                        # If there is a raw file for this run, output it; otherwise output the processed file directly
                        if run_has_raw[index]:
                            # raw DataFiles were appended before processed ones; find the raw file at its index
                            raw_df_index = sum(run_has_raw[:index])  # how many raw files precede this run
                            process.outputs.append(assay_obj.data_files[raw_df_index])
                        else:
                            process.outputs.append(assay_obj.data_files[run_processed_df_index[index]])
                    ...
                    # Use per-assay processing_protocols entries (from front-end) as ParameterValue(s)
                        ...
                        process = Process(executes_protocol=protocol, parameter_values=parameter_list)
                        process.name = f"{sensor_alias}_run_{run_number}_processing"
                        # Input: raw file if it exists, otherwise the processed file from the measurement step
                        if run_has_raw[index]:
                            raw_df_index = sum(run_has_raw[:index])
                            process.inputs.append(assay_obj.data_files[raw_df_index])
                        else:
                            process.inputs.append(assay_obj.data_files[run_processed_df_index[index]])
                        process.outputs.append(assay_obj.data_files[run_processed_df_index[index]])
                        assay_obj.process_sequence.append(process)

            # Link measurement -> processing process for each run independently.
            # process_sequence is ordered [meas_0, proc_0, meas_1, proc_1, ...]
            # so pairs are at indices (2*i, 2*i+1). Linking across run boundaries
            # (proc_i -> meas_i+1) would imply a false causal dependency.
            for i in range(len(runs)):
                meas_idx = 2 * i
                proc_idx = 2 * i + 1
                if meas_idx < len(assay_obj.process_sequence) and proc_idx < len(assay_obj.process_sequence):
                    plink(assay_obj.process_sequence[meas_idx], assay_obj.process_sequence[proc_idx])
