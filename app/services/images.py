...
def _safe_archive_path(original_name: str, attachment_id: str, extension: str) -> str:
    stem = re.sub(r"[^A-Za-z0-9._-]+", "-", Path(original_name).stem).strip(".-") or "test-setup"
    safe_id = re.sub(r"[^A-Za-z0-9_-]+", "", attachment_id)[:16] or "image"
    return f"Images/{stem}-{safe_id}.{extension}"
...
async def prepare_images(
    ...
    for upload in uploaded_files:
        ...
        if len(content) > MAX_IMAGE_BYTES or total_bytes > MAX_TOTAL_IMAGE_BYTES:
